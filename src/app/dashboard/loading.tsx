        ...
        <main className="flex-1 overflow-y-auto bg-[#F7F6FF]">
          <div className="mx-auto max-w-3xl px-6 py-8">
            ...
            <div className="mb-6 flex animate-pulse items-center justify-between">
              <div className="h-7 w-36 rounded-xl bg-black/[0.08]" />
              <div className="h-9 w-32 rounded-full bg-black/[0.08]" />
            </div>

            {/* Stat cards */}
            <div className="mb-6 grid animate-pulse grid-cols-3 gap-4">
              {[0, 1, 2].map((i) => (
                <div key={i} className="h-20 rounded-2xl bg-black/[0.07]" />
              ))}
            </div>
            ...
            <div className="mb-4 flex animate-pulse gap-2">
              {[0, 1, 2].map((i) => (
                <div key={i} className="h-8 w-20 rounded-full bg-black/[0.07]" />
              ))}
            </div>
            ...
            <div className="animate-pulse space-y-3">
              {[0, 1, 2].map((i) => (
                <div key={i} className="h-28 rounded-2xl bg-black/[0.07]" />
              ))}
            </div>
          </div>
        </main>
